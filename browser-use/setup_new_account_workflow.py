...
class LLM:
    ...
    def call(self, messages):
        ...
        try:
            ...
            options = []
            for i in range(10):
                options.append({"tweet_text": f"hmm {i + 1}."})
    ...
    def follow_accounts(self):
        try:
            with open(os.path.join(SCRIPT_DIR, pathToData + users), "r") as f:
                ...
                if data:
                    filtered_data = {
                        user["handle"]: user["score"]
                        for user in data
                        if user["score"] != 100
                        and not user.get("alreadyFollowingOrBlocked", False)
                    }
                    print(filtered_data)
                    ...
                    for user_handle in filtered_data.keys():
                        ...
                        with open(
                            os.path.join(SCRIPT_DIR, pathToData + users), "w"
                        ) as f:
                            json.dump(data, f, indent=2)
                    return True
            return False
            ...
            with open(os.path.join(SCRIPT_DIR, pathToData + users), "r") as f:
                ...
                if data:
                    ...
                    filtered_data = {
                        user["handle"]
                        for user in data
                        if list_name not in user["memberOfTheseLists"]
                        and user["score"] != 100
                    }  # Only check this condition

                    print(
                        "Filtered data (only checking 'alreadyFollowingOrBlocked'):",
                        filtered_data,
                    )

                    from my_twitter_api_v3.lists.add_members_to_list import (
                        add_members_to_list,
                    )
